...
class QAgent:
    ...
    def _apply_update(self, prev_state: int, action: int, new_state: int,
                      reward: Number) -> None:
        self._Q[prev_state, action] = calculate_bellman_update(
            self._Q[prev_state, action], np.max(self._Q[new_state, :]), reward,
            self.gamma, self.alpha)

    def _run_episode(self, callback: Callable[int, int, int, None]) -> float:
